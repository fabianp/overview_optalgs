...
*/
function genIterDiagram(f, xstar, axis, optMethod, showSolution, showStartingPoint, totalIters) {
  ...
  */
  ...
  function renderIterates(div) {
    ...
    function iter(alpha, beta, w0) {
      ...
      // Generate iterates
      if (optMethod != null) {
        var OW = optMethod(f, w0, alpha, totalIters)
        var W = OW[1]

        update2D(W)
        
        updateSol(OW[1][OW[1].length - 1][0], OW[1][OW[1].length - 1][1])
