...
function runGradientDescent(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1]
  W.push(w0); Obj.push(fx[0])
  for (var i = 0; i < totalIters; i++) {
    var w = numeric.add(w, numeric.mul(-alpha, gx))
    fx = f(w); gx = fx[1]
    if (w.every(isFinite)) {
      W.push(w); Obj.push(fx[0])
    } else{ break; }
...
function runGradientDescentLS(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1]
  W.push(w0); Obj.push(fx[0])
  for (var i = 0; i < totalIters; i++) {
    alpha = alpha * 1.1;
    if (numeric.dot(gx, gx) < 10e-6) {
      break;
    }
    for (var j=0; j < 10; j++) {
      var w_next = numeric.add(w, numeric.mul(-alpha, gx))
      var rhs = fx[0] - (0.5 * alpha) * numeric.dot(gx, gx);
      if (f(w_next)[0] <= rhs) {
        break;
      } else {
        alpha = alpha / 2.;
...
function runStochasticGradient(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1]
  // simulate random sampling -- ugly
  gx[0] = gx[0] + (Math.random() - 0.5) * 4
  gx[1] = gx[1] + (Math.random() - 0.5) * 4
  W.push(w0); Obj.push(fx[0])
  for (var i = 0; i < totalIters; i++) {
    var w = numeric.add(w, numeric.mul(-alpha, gx))
    fx = f(w); gx = fx[1]
...
function runStochasticGradientDecreasingSteps(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1]
  // simulate random sampling -- ugly
  gx[0] = gx[0] + (Math.random() - 0.5) * 2
  gx[1] = gx[1] + (Math.random() - 0.5) * 2
  W.push(w0); Obj.push(fx[0])
  for (var i = 0; i < totalIters; i++) {
    var w = numeric.add(w, numeric.mul(-alpha / Math.sqrt(i+1), gx))
    fx = f(w); gx = fx[1]
    gx[0] = gx[0] + (Math.random() - 0.5) * 2
    gx[1] = gx[1] + (Math.random() - 0.5) * 2
    if (w.every(isFinite)) {
...
function runNewton(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1]
  var H = fx[2];
  W.push(w0); Obj.push(fx[0])
  // var beta = float(1)
  for (var i = 0; i < totalIters; i++) {
    var eigval = numeric.eig(H).lambda.x[1]
    if (eigval <= 0) {
      H[0][0] = H[0][0] - 2 * eigval + 0.0001;
      H[1][1] = H[1][1] - 2 * eigval + 0.0001;
    }
    if (eigval == 0) {
      H[0][0] = H[0][0] + 0.00001;
      H[1][1] = H[1][1] + 0.00001;
    }
    var nn = numeric.mul(-alpha, numeric.inv(H));
    var w = numeric.add(w, numeric.dot(nn, gx));
    // debugger
    // var w = numeric.add(w, numeric.mul(-alpha, gx))
    // var w = numeric.add(w, numeric.dot(-nn, gx))
    fx = f(w); gx = fx[1]; H = fx[2];
    if (w.every(isFinite)) {
      W.push(w); Obj.push(fx[0])
    } else{ break; }
  }
...
function runBFGS(f, w0, alpha, totalIters) {
  var Obj = []; var W = []; var z = zeros(w0.length); var w = w0
  var fx = f(w0); var gx = fx[1];
  var H = [[1, 0], [0, 1]];
  W.push(w0); Obj.push(fx[0])
  ...
  for (var i = 0; i < totalIters; i++) {
    var eigval = numeric.eig(H).lambda.x[1]
    // if (eigval <= 0.00001) {
    //   H[0][0] = H[0][0] - eigval + 0.0001;
    //   H[1][1] = H[1][1] - eigval + 0.0001;
    // }
    // if (eigval == 0) {
    //   H[0][0] = H[0][0] + 0.000001;
    //   H[1][1] = H[1][1] + 0.000001;
    // }
    var invH = numeric.inv(H);
    var nn = numeric.mul(-alpha, invH);
    var sk = numeric.dot(nn, gx);
    var w = numeric.add(w, sk);
    ...
    fx = f(w); 
    // update hessian
    var yk = numeric.sub(fx[1], gx);
    var term1 = outerProduct(yk, yk);
    term1 = numeric.mul(1./(numeric.dot(yk, sk)), term1);
    var Hsk = numeric.dot(H, sk);
    var term2  = numeric.mul(-1./numeric.dot(sk, Hsk), outerProduct(Hsk, Hsk));
    if (numeric.dot(yk, sk) > 0) {
      var H = numeric.add(numeric.add(H, term1), term2);
    } else{
      H = [[1, 0], [0, 1]];
    }
    gx = fx[1];
...
function bananaf(xy) {
  ...
  var d2fx = [ [ 2 + 160*x*x - 80*(-x*x + y), -s * 80*x], [ -s * 80*x, s * 40  ] ]
...
function quadf(xy) {
  ...
  var d2fx = A //numeric.dot(numeric.transpose(A), A)
  return [fx, dfx, d2fx]
}

/* Identity */
function well_conditioned_quad(xy) {
  var U = givens(0)
  var lambda = numeric.diag([1,2])
  var A = numeric.dot(numeric.transpose(U),numeric.dot(lambda, U))
  var dfx = numeric.dot(A,xy)
  var fx  = 0.5*numeric.dot(dfx,xy)
  var d2fx = A //numeric.dot(numeric.transpose(A), A)
